function [ allName, Sequence ] = block3_distractor(ppath, S )
...
for i=1:20
    ...
    elseif i>5 && i<11
        Location(1,i)=distractorPresLocation(i-4);
    elseif i>10 && i<15
        Location(1,i)=T2Location(i-10);
    else
        Location(1,i)=distractorPresLocation(i-8);
    end
end
g=i-12;
T1remainPresentation=[T1Location((4+1):end), distractorPresLocation(g+1:(end-g)/2+g)]; % car target not at switch site, total 18
T1remainPresentation=Shuffle(T1remainPresentation);
T2remainPresentation=[T2Location((4+1):end), distractorPresLocation((end-g)/2+g+1:end)]; % dog target not at switch site, total 18
